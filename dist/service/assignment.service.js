...
class AssignmentService {
    // CREATE ASSIGNMENT
    async insertAssignment(data, loggedInUserId) {
        ...
        if (!allocation) {
            throw new Error("You are not assigned to this subject/semester/faculty. Cannot create assignment.");
        }
        // 3. Create assignment
    ...
    async updateAssignment(id, assignment) {
        ...
        if (assignment.teacherId !== undefined)
            updateData.teacherId = assignment.teacherId;
        if (assignment.subjectId !== undefined)
            updateData.subjectId = assignment.subjectId;
        if (assignment.facultyId !== undefined)
            updateData.facultyId = assignment.facultyId;
        if (assignment.semesterId !== undefined)
            updateData.semesterId = assignment.semesterId;
    ...
    // DELETE ASSIGNMENT
    async deleteAssignment(id, loggedInUserId) {
        const assignment = await prisma.assignment.findUnique({ where: { id } });
        if (!assignment)
            throw new Error("Assignment not found");
